...
- **`globals.css` imports Tailwind in three pieces, unlayered, and must stay that way.** `@neondatabase/auth` ships its own Tailwind build *without* a cascade layer, and an unlayered rule beats every layered one regardless of specificity or order — so its preflight (`*{border:0 solid}`, `h1{font-size:inherit}`) silently won over our `@layer utilities`. Cards and pills rendered borderless and `text-4xl` rendered at 18px, on every screen, while `pnpm test` passed: the `navegador` project imports `globals.css` through Vite, so it never sees the vendor sheet. Restoring `@import "tailwindcss"` brings the bug back. A change to how a utility lands on the page has to be checked in the running app, not only in the suite.
